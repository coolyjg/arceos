...
AR = $(AX_ARCH)-linux-musl-ar

redis-dir = $(APP)/redis/

ARCEOS_ROOT = $(PWD)
MY_REDIS_LDFLAGS = -nostdlib -static -T$(ARCEOS_ROOT)/modules/axhal/linker_$(AX_PLATFORM).lds
MY_REDIS_CFLAGS = $(CFLAGS) -Wno-unused-parameter -Wno-int-conversion -Wno-stringop-overflow -Wno-restrict -Wno-format -Wno-unused-variable -Wno-maybe-uninitialized -nostdinc -fno-builtin -I$(ARCEOS_ROOT)/ulib/axlibc/include
MY_CFLAGS = -static -no-pie $(CFLAGS)

export ARCEOS_ROOT

ifeq ($(AX_ARCH), aarch64)
MY_CFLAGS += -mno-outline-atomics
else ifeq ($(AX_ARCH), x86_64)
...
$(pre-objs):
	...
	cd $(APP)/redis && CC=$(AX_ARCH)-linux-musl-gcc USE_JEMALLOC=no $(MAKE) -j V=1 REDIS_CFLAGS='$(MY_REDIS_CFLAGS)' REDIS_LDFLAGS+='$(MY_REDIS_LDFLAGS)' CFLAGS="$(MY_CFLAGS)" LDFLAGS="-static -no-pie"

$(APP)/redis_server.o: $(pre-objs)
	$(AX_ARCH)-linux-musl-ld -r -o $@ $^
